...
- 2026-04-28: Plan-time scope decisions: (a) form-control migrations partially expanded — Badge sites + thesis/brief textareas planned, but textareas deferred when it became clear they need a SectionField molecule that DS-07 wouldn't deliver. (b) `StatusBadge` made `onChange` optional so the same molecule covers read-only Badge replacements (avoided a separate `Pill` molecule). (c) FloatingMenu adoption sites narrowed: `create-mission-modal` typeahead and `ideas-list` tag-picker dropped from scope (different patterns; future molecules). (d) Multi-step / form-wrapped Modal footers (`create-segment-modal`, `create-mission-modal`, `create-offer-modal`, `create-project-modal`, `create-platform-modal`) left inline — `footer` slot can't sit inside a `<form>` for `type="submit"` semantics; per-step phase footers also don't fit cleanly.
- 2026-04-28: Build complete. Shipped: 6 new molecules (FloatingMenu, ActionMenu, ListItem, FilterPill, FilterPillGroup, ArchiveItemModal), 2 existing-molecule changes (Modal `footer?` slot, StatusBadge tag-hue variant + optional onChange + FloatingMenu refactor). Migrated mission-workspace + project-workspace status pickers to StatusBadge; dashboard rows + recent activity + input queue to ListItem; ideas-list filter pills to FilterPillGroup; entity-outcomes-panel to ActionMenu; 3 archive flows (segment / platform / offer) to ArchiveItemModal (offer with redirectTo→nextId adapter); 6 Badge sites to read-only StatusBadge (Draft / Archived / Inactive); customer-journey-panel + project-workspace 2 modals + create-application-modal to Modal `footer` slot. Deleted 3 archive-*-modal files. Coherence-check warnings: 21 atom imports → 10 (all 10 with explicit `// DS-07 exception:` comments naming their reason). NewInputDropdown extracted to its own client component file (`app/(dashboard)/new-input-dropdown.tsx`) to fix server→client boundary error: Lucide `Plus` function reference can't serialise across server-component boundaries.
- 2026-04-28: DS-02 state-token retune (separate concern surfaced during DS-07 build): `--success-bg` / `--warning-bg` / `--error-bg` / `--info-bg` and their `--*-foreground` pairs all retuned to harmonise with the brand sage/dusty palette. Old defaults (Tailwind pale-pastel) replaced with: success = sage-deepened (`#6B9A7E` / `#D6E5DC` / `#2D4A38`), warning = gold (`#C59B6C` / `#F0DFC8` / `#6B4A20`), error = deep red (`#AF1B1B` / `#F2D1D1` / `#5A0E0E`), info = dusty blue matching `--tag-1` (`#A1BED6` / `#D5E2EC` / `#2A4A63`). Edited `02-app/app/globals.css` only — every StatusBadge in the app picks up the new palette automatically. Required `rm -rf .next` to clear Turbopack's compiled CSS cache before the new tokens took effect.
- 2026-04-28: Manual verification: 12/12 test cases passed (mission/project status pickers, dashboard ActionMenu + ListItem rows, ideas-list filter pills, archive flows for platform/segment/offer, Modal footer slot in 3+ places, entity-outcomes-panel popover, Badge sites). Brief status → `complete`. Backlog DS-07 → `done`. Foundation programme: complete (DS-01..DS-09).
