...
**Choosing the ID:** if the idea is a refinement, follow-on, or expansion of an existing feature, prefer a lineage suffix (e.g. `DNA-07b`, `OUT-02c`) over a fresh top-level ID. The suffix preserves the relationship in the backlog reading order and makes the connection explicit when the entry is read cold. Use a fresh top-level ID only when the idea is genuinely a new concern.
