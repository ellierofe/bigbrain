...
8. `01-design/design-system.md` — if this feature has any UI output, read the full spec before planning. Note which molecule components are available, which are needed, and whether any new ones will be required.
9. The layout spec's **Molecule composition** section (in the brief's UI/UX notes or `01-design/wireframes/[feature-id]-layout.md`). This section lists the existing molecules to compose with and any new molecules required (with one-line spec sketches). It is the input to Step B's molecule plan and Step C's hard gate. If the layout spec lacks this section, stop — return to `layout-design` to add it before continuing.
10. `02-app/components/registry.ts` — verify what's actually available and at what import path.
...
**Molecule plan** *(mandatory if the feature has UI output)*

Restate the layout spec's molecule composition in build-actionable form:

- **Existing molecules to compose with** — list each, note the import path from `02-app/components/registry.ts`. The build will import these. No new code for them.
- **Existing molecules to extend** — if a registered molecule is close but needs a new prop or variant: list it, name the change, confirm the change is consistent with its spec in `01-design/design-system.md` (or note "molecule has no spec — see DS-03; spec it now as part of this build to close the gap").
- **New molecules to create** — list each. For every entry here:
  1. A spec sketch must already exist in the layout spec's Molecule composition section. Restate it.
  2. Before plan approval (Step C), invoke `design-system` Mode A on each new molecule and present its draft spec entry. The plan gate is **blocked** until each draft spec is approved and ready to land in `01-design/design-system.md`.
  3. Note where the new molecule lives (`02-app/components/[name].tsx`) and what registry entry will be added.
- **Atoms used directly** — should be empty. If any `components/ui/*` atom would be composed directly in an organism, that's a missing molecule — promote it to "New molecules to create" above. Direct atom use is allowed only with explicit justification ("genuinely one-off, not worth a molecule") and only if the layout spec already justified it.

This subsection is the central artefact for stopping drift. If it's incomplete or honest about needing a spec, the plan gate cannot pass.
...
Before presenting the plan, run the **molecule plan check**:

- Every entry under "New molecules to create" in Step B's Molecule plan must have a draft spec ready (via `design-system` Mode A). If any entry lacks a draft spec, draft it now and present it as part of the plan — do not present the plan with bare "TBD" molecule entries.
- Every "Existing molecules to extend" entry must reference its current spec (or note the missing-spec gap and provide a draft for it as part of this build).
- The "Atoms used directly" list must be empty, OR every entry must have explicit justification ("genuinely one-off, not worth a molecule") that has been recorded in the layout spec.

If any of those conditions fail: stop. Resolve the molecule plan first — either by drafting the missing specs (Mode A), bouncing back to `layout-design` to add what's missing, or surfacing the gap to the human for a decision. Plan approval cannot proceed past an incomplete molecule plan.

Once the molecule plan is complete, present the full plan. State explicitly: "No code has been written. Approve this plan to proceed."
...
- Push back on a molecule decision → re-draft the spec or restructure the molecule plan and re-present
...
- **Server-only registry → client-safe summary projection (from OUT-01b build, 2026-05-04):** When a client component needs data from a server-only registry — anything that imports `node:fs` (brief loaders, file-based catalogues) or other Node-only APIs transitively, including `lib/skills/registry`, `lib/llm/content/*` registries, prompt-fragment loaders, etc. — do NOT import the registry from the client. The Turbopack/Next.js bundler will pull `node:fs` into the client bundle and the build fails (or worse, fails only at prerender). Instead: define a client-safe summary type in a shared types module, write a server-side projection helper (e.g. `toSkillSummary(skill: Skill): SkillSummary`), call it from the server page/component, and pass the projected data down via props. The pattern is established at `02-app/lib/chat-context-pane/skill-summary.ts` + `02-app/lib/chat-context-pane/types.ts` (the `SkillSummary` type). Surface this early — discovery via build failure costs a mid-build refactor.

**React/TS conventions (from MISSION-01 build):**
- `useRef` with timers: `useRef<ReturnType<typeof setTimeout> | null>(null)` — always pass initial `null`. Guard clears: `if (ref.current) clearTimeout(ref.current)`. Cleanup returns need braces: `return () => { if (ref.current) clearTimeout(ref.current) }`.
- Radix primitives use the `render` prop pattern, not `asChild`. E.g. `<DropdownMenuTrigger render={<Button>...</Button>} />` not `<DropdownMenuTrigger asChild>`.
- `@/components/ui/label` does not exist — use plain `<label className="text-sm font-medium">` instead.

**List view checklist:**
- Empty state must include a create CTA (button that opens the create modal). Don't render the empty state from the server component without the create action — always render the client component so the modal trigger is available.
...
**Design system gate (from DS-01 / DS-08):**
The molecule-level hard gate runs at **plan time** (Step B Molecule plan + Step C plan-approval check), not during build. By Step D, every molecule used in this feature is either: an existing spec'd molecule (compose it), an existing molecule with an approved extension (extend it), or a new molecule with an approved spec (build it then compose it). If something unexpected requires a molecule that wasn't in the plan, **stop and surface it** — do not improvise an inline-styled component.

While building, follow the spec exactly:
- New molecules: implement to the approved spec, add to `01-design/design-system.md` and `02-app/components/registry.ts` before composing into the organism.
- No hardcoded colour or spacing values: tokens only (`--primary`, `--muted-foreground`, `--border`, etc.). No raw hex / rgb. Use semantic state tokens (`--color-success`, `--color-warning`, `--color-error`) when DS-02 has landed; until then, flag the colour as a known gap rather than reaching for `green-*` / `amber-*`.
- No Tailwind appearance classes (`bg-*`, `text-*`, `border-*`, `shadow-*`) in organism-layer files. "Organism layer" includes both `app/(dashboard)/**` AND any file in `02-app/components/` matching `*-view.tsx`, `*-modal.tsx`, `*-panel.tsx`, `*-list.tsx`, `*-table.tsx`, `*-workspace.tsx`, `*-area.tsx`, `*-section.tsx`, `*-card.tsx` — anything composing molecules into a feature surface, regardless of directory. Excludes `02-app/components/ui/*` and any file declared in `components/registry.ts`.
