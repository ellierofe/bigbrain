...
- `launch-picker-grid-template.md` — filterable card grid where each card is a launchpad to a workspace (not a CRUD detail view). Multi-faceted filters (URL-state), brand-scoped favourites, lockable cards driven by a `prerequisites` jsonb column. For catalogue rows the user *picks to launch*, not items they own. Established from OUT-02-P4a (content-creator picker at `/content`).
- `creator-workspace-three-region-template.md` — fixed-viewport generation workspace: 380px left rail (input SectionCards + lighter Settings strip + sticky primary action) beside a flex-1 right pane (empty / loading / success / error states). For parameter-driven generation surfaces — LLM, render, compute. Workspace is transient; output goes elsewhere. Requires ≥ 1024px viewport. Established from OUT-02-P4a (content-creator generation surface at `/content/create/[slug]`).
- `chat-context-pane-template.md` — adaptive context pane mounted as a right-side column inside the chat page: 56px vertical icon rail (tab strip) + scrollable content panel + resize/collapse. Pluggable tabs render conversation-scoped context (skill state, future: referenced item, topic, variant context). Persistence is per-brand (open/width) + per-conversation (selected tab). Established from OUT-01b (skill-state tab as v1's only tab). Future tabs: any conversation-scoped context surface inside `/chat`.
